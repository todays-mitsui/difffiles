...
const saveDeletedList = (destpath, dirname, deleted) => {
  ...
  const data = `削除されたファイル数: ${ deleted.length }${ EOL }${ EOL }${ deleted.map(f => path.resolve(CONF.cwd, f)).join(EOL) }
`;

  fs.mkdirpSync(destpath);

  return new Promise((resolve, reject) => {
    fs.writeFile(path.resolve(destpath, filename), data, function (error) {
      if (error) {
        reject();
